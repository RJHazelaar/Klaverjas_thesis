...
def test_vs_rule_player(
    ...
    process_id: int,
    mcts_params: dict,
    model_paths_value: List[str],
    model_paths_policy: List[str],
):
    # random.seed(13)
    alpha_eval_time = 0
    ...
    model_value = None
    model_policy = None
    if model_paths_value[0] is not None:
        try:
            model_value = tf.keras.models.load_model("/local/s1762508/Klaverjas_thesis-main/Data/Models/" + model_paths_value[0])
            #model = tf.keras.models.load_model(parent_dir+"Data/Models/" + model_paths[0])
        except:
            print("model not found")
            raise Exception("model not found")
        
    if model_paths_policy[0] is not None:
        try:
            model_policy = tf.keras.models.load_model("/local/s1762508/Klaverjas_thesis-main/Data/Models/" + model_paths_value[0])
            #model = tf.keras.models.load_model(parent_dir+"Data/Models/" + model_paths[0])
        except:
            print("model not found")
            raise Exception("model not found")
...
def run_test_multiprocess(
    n_cores: int, opponent: str, total_rounds: int, mcts_params: dict, model_paths_value: List[str], model_paths_policy: List[str], multiprocessing: bool
):
    ...
    if multiprocessing:
        with get_context("spawn").Pool(processes=n_cores) as pool:
            results = pool.starmap(
                test_function,
                [(rounds_per_process, i, mcts_params, model_paths_value, model_paths_policy) for i in range(n_cores)],
            )
        ...
        scores_round, points_cumulative, mcts_times, alpha_eval_time = test_function(
            rounds_per_process, 0, mcts_params, model_paths_value, model_paths_policy
        )
